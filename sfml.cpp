...
using namespace sf;
    ...
    pthread_t main_thread, boostThread;

pthread_t scoreThread, ProducePallets;
bool shouldExit = false, start = true;
string action = "";
char prevDirection = ' ', directon[] = {'U', 'D', 'R', 'L'};
int uSleepTime = 200000, score = 0, lives = 3;
int uSleep = 100000, extratime = 10500, n = 9, m =9;
...
float PowerPalletTime = 7.0f; // Loop duration in seconds
...
sem_t requestBoost, ing1, ing2, ing3, ing4;
Clock boostTimeLimit;
...
void* generateSpeedBoost(void* p)
{
    while (!shouldExit)
    {
        sem_wait(&requestBoost);

        // if request generated, then create the ingredient

        int produceIngX = rand() % 4;
        switch(produceIngX)
        {
            case 0:
                cout << "produced ing1" << endl;
                sem_post(&ing1);
            break;
            case 1:
                cout << "produced ing2" << endl;
                sem_post(&ing2);
            break;
            case 2:
                cout << "produced ing3" << endl;
                sem_post(&ing3);
            break;
            case 3:
                cout << "produced ing4" << endl;
                sem_post(&ing4);
            break;
        }
    }
...
int random_move(ghostStruct &ghost, int prevDirec)
{
    ...
    if (prevDirec==0  && (grid[ghost.ghostGridY - 1][ghost.ghostGridX] != 0))
        {
            ghostup(ghost);
            return 0;
        }
        else if (prevDirec ==1 && (grid[ghost.ghostGridY + 1][ghost.ghostGridX] != 0))
        {
            //  go down
            ghostdown(ghost);
            return 1;
        }
        else if ( prevDirec ==2 && (grid[ghost.ghostGridY ][ghost.ghostGridX -1] != 0))
        {
            //  go left
            ghostleft(ghost);
            return 2;
        }
        else if (prevDirec ==3 && (grid[ghost.ghostGridY ][ghost.ghostGridX +1] != 0))
        { //  go right
            ghostright(ghost);
            return 3;
        }
...
int speedGhostUp(ghostStruct& ghost, int n)
{  
    if (ghost.hasBoost) 
    {
        if (boostTimeLimit.getElapsedTime().asSeconds() >= 5.0f) 
        {
            cout << "boost usage time over!" << endl;
            sem_post(&requestBoost);
            ghost.hasBoost = false;
            ghost.ghostSprite.setColor(ghost.color);
           return 9;        
        }
        ...
        if (ghost.fast)
        {
            // if ghost is fastGhost then it can go fast  
            ghost.ghostSprite.setColor(Color::Green);  
            return -9;  
        }
        else 
        {
...
void *ghost1Movement(void *arg)
{
    ...
    int prevDirec, directionInt = 0, actualMovement = 0, checkIfIngAvailable = 0;

    while (!shouldExit)
    {
        speedGhostUp(ghost1, n);
        sem_getvalue(&ing1, &checkIfIngAvailable);
        if (checkIfIngAvailable != 0)
        {
            sem_wait(&ing1);
            ghost1.hasBoost = true;
            cout << "ghost1 has ing1" << endl;
            boostTimeLimit.restart();
        }
...
void *ghost2Movement(void *arg)
{
    ...
    while (!shouldExit)
    {
        n= speedGhostUp(ghost2, n);
        sem_getvalue(&ing2, &checkIfIngAvailable);
        if (checkIfIngAvailable != 0)
        {
            sem_wait(&ing2);
            ghost2.hasBoost = true;
            cout << "ghost2 has ing2" << endl;
            boostTimeLimit.restart();
        }
        ...
        if (tickets[1])
        {
            ...
            else if (!ghost2.eaten) //&& !respawned)
            {
...
void *ghost3Movement(void *arg)
{
    ...
    while (!shouldExit)
    {
        speedGhostUp(ghost3, n);
        sem_getvalue(&ing3, &checkIfIngAvailable);
        if (checkIfIngAvailable != 0)
        {
            sem_wait(&ing3);
            ghost3.hasBoost = true;
            cout << "ghost3 has ing3" << endl;
            boostTimeLimit.restart();
        }
...
void *ghost4Movement(void *arg)
{
    ...
    int prevDirec = 0, directionInt = 0, actualMovement = 0, checkIfIngAvailable = 0;

    while (!shouldExit)
    {
        m= speedGhostUp(ghost4, m);
        sem_getvalue(&ing4, &checkIfIngAvailable);
        if (checkIfIngAvailable != 0)
        {
            sem_wait(&ing4);
            ghost4.hasBoost = true;
            cout << "ghost4 has ing4" << endl;
            boostTimeLimit.restart();
        }
        ...
        if (tickets[3])
        {
            ...
            else if (!ghost4.eaten) //&& !respawned)
            {
...
void *mainthread(void *p)
{
    ...
    pthread_create(&boostThread, nullptr, &generateSpeedBoost, nullptr);
...
void *sfmlWindow(void *p)
{
    ...
    pthread_create(&scoreThread, nullptr, &incScore, nullptr);
     ...
     sf::Text title("MAIN MENU", font, 50);
        ...
        sf::Text option1("PLAY GAME", font, 30);
        option1.setFillColor(sf::Color::Green);
        option1.setPosition(420, 300);

        sf::Text option2("EXIT GAME", font, 30);
        option2.setFillColor(sf::Color::Red);
        option2.setPosition(420, 350);
